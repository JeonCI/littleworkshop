				...
				<tbody>
					...
					<c:forEach var="item" items="${noticeList}">
						<tr>
							...
							<td><a href="noticeView/${item.noticeCode}">[${item.noticeCategory}] ${item.noticeSubject }</a></td>
							<td><fmt:formatDate value="${item.noticeRegDate}" pattern="yyyy-MM-dd"/></td>
							<td><a href="noticeUpdate/${item.noticeCode}">수정</a> / <a href="noticeDelete/${item.noticeCode }">삭제</a></td>
						</tr>
					</c:forEach>
					</c:if>
				</tbody>
