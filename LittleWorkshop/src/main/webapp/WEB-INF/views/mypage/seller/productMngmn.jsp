	...
	<div>
		...
		<form method="get" id="searchForm">
			...
			<c:if test="${categoryList.size() > 0 }">
				<ul>
				<li class="category ${pager.search==0?'click':''}" onclick="setCategory(this);"><a href="./productMngmn">전체</a></li>
				<c:forEach var="category" items="${categoryList}">
					<li class="category ${category.productCategoryCode == pager.search?'click':''}" onclick="setCategory(this);" value="${category.productCategoryCode}"><a href="./productMngmn?search=${category.productCategoryCode}">${category.productCategory}</a></li>
				</c:forEach>
				</ul>
			</c:if>
		</form>
